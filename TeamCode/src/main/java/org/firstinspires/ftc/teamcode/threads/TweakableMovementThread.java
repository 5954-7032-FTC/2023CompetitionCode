...
public class TweakableMovementThread extends RobotThread {
    ...
    private final Tweakable[] tweakables;
    ...
    public void run() {
        while (!isCancelled()) {
            ...
            if (_gamepad.right_trigger > 0.2) {
                drive.moveRect(
                        _Joy1Y.ramp(deadzone(_gamepad.left_stick_y*_fine_control.value,_zone_forward.value),_ramp_rate.value),
                        _Joy1X.ramp(deadzone(_gamepad.left_stick_x*_fine_control.value,_zone_lateral.value),_ramp_rate.value),
                        _Joy2X.ramp(deadzone(_gamepad.right_stick_x* _fine_control.value,_zone_rotation.value),_ramp_rate.value)
                );
            }

            else {
                drive.moveRect(
                        _Joy1Y.ramp(deadzone(_gamepad.left_stick_y, _zone_forward.value),_ramp_rate.value),
                        _Joy1X.ramp(deadzone(_gamepad.left_stick_x, _zone_lateral.value),_ramp_rate.value),
